...
import com.rexi.pkty.service.DichVuGroq;
import com.rexi.pkty.service.DichVuGemini;
import com.rexi.pkty.service.DichVuBoNhoAi;
...
public class TroChuyenController {

    private static final Logger logger = Logger.getLogger(TroChuyenController.class.getName());

    @Autowired
    private DichVuGroq groqService;

    @Autowired
    private DichVuGemini geminiService;

    @Autowired
    private DichVuBoNhoAi aiMemoryService;
    ...
    public Map<String, String> chat(
            @RequestBody List<TinNhanChat> history,
        ...
        if (rateLimiter.size() > 1000) {
            ...
            // CẢNH BÁO OOM: Chống tràn RAM nếu bị tấn công DDoS Botnet (Nhiều IP cùng lúc)
        ...
        try {
            // BẢO MẬT LỚP 2: Chống Prompt Injection (System Spoofing)
            // Kẻ tấn công có thể chèn các tin nhắn mang role "system" từ Frontend để hack/ghi đè persona của AI.
            // Phải xóa sạch các tin nhắn system giả mạo do Client gửi lên.
            if (history != null) {
            ...
            TinNhanChat lastMsg = history.get(history.size() - 1);
            ...
            nhatKyHeThongService.logActionWithUsername(realUsername, "TRÒ CHUYỆN", "TroChuyen", "Khách hàng hỏi AI: " + userQuery);
            ...
            TinNhanChat systemMsg = new TinNhanChat();
