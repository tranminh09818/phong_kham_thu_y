...
public class SchedulingService {
    ...
    /**
     * Tự động gửi Email nhắc nhở lúc 08:00 sáng cho các lịch hẹn ngày mai.
     * BUG FIX: Chỉ dùng enum tiếng Anh (CHO_XAC_NHAN, DA_XAC_NHAN) cho nhất quán,
     * bỏ N'Chờ xác nhận' tiếng Việt dư thừa (data cũ trong DB nếu có sẽ không match).
     */
     ...
     * → KHÔNG BAO GIỜ có hóa đơn nào được match → email nhắc nợ không bao giờ được gửi!
    ...
    public void autoReportDailyDebt() {
        try {
            // Dùng UPPER() để so sánh case-insensitive, bảo vệ khỏi data không nhất quán
