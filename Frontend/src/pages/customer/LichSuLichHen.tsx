  ...
  if (loading) {
    return (
      <div className="animate-fade-in">
        {/* Khung xương Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', padding: '56px 48px', borderRadius: 'var(--radius-xl)', background: 'var(--secondary-gradient)', color: 'white', opacity: 0.15 }}>
          <div>
            <div style={{ height: '40px', width: '300px', background: 'white', opacity: 0.3, borderRadius: '12px', marginBottom: '12px' }}></div>
            <div style={{ height: '20px', width: '400px', background: 'white', opacity: 0.2, borderRadius: '8px' }}></div>
          </div>
        </div>

        {/* Khung xương danh sách lịch hẹn */}
        <div style={{ display: 'grid', gap: '28px' }}>
          {[1, 2, 3].map((i) => (
            <div key={i} className="glass-card" style={{ padding: '24px 32px', borderRadius: 'var(--radius-xl)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid var(--gray-100)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                <Skeleton width="56px" height="56px" borderRadius="16px" />
                <div>
                  <Skeleton width="200px" height="24px" borderRadius="8px" style={{ marginBottom: '8px' }} />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '32px' }}>
                ...
                <Skeleton width="110px" height="32px" borderRadius="50px" />
                <Skeleton width="40px" height="40px" borderRadius="50px" />
              </div>
            </div>
          ))}
        </div>
