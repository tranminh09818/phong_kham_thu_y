...
export const TieuDeChatbot: React.FC<{
    ...
    <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontWeight: 900, fontSize: isMobile ? '1rem' : '1.05rem', letterSpacing: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '10px' : '14px' }}>
            <span
                ...
                style={{ fontSize: isMobile ? '21px' : '22px', cursor: 'pointer', color: isVoiceEnabled ? '#4ade80' : 'white', opacity: isVoiceEnabled ? 1 : 0.7 }}
                onClick={onToggleVoice}
                title={isVoiceEnabled ? "Tắt đọc thành tiếng" : "Bật đọc thành tiếng"}
            >
                {isVoiceEnabled ? 'volume_up' : 'volume_off'}
            </span>
            <span className="material-symbols-outlined" style={{ fontSize: isMobile ? '21px' : '22px', cursor: 'pointer', opacity: 0.8 }} onClick={onResetChat} title="Làm mới cuộc hội thoại">
                restart_alt
            </span>
            <span className="material-symbols-outlined" style={{ fontSize: isMobile ? '21px' : '22px', cursor: 'pointer', opacity: 0.8 }} onClick={onClose}>
                close
            </span>
        </div>
    </div>
