 ...
 * TRÌNH PHÁT LOTTIE
 */
export const TrinhPhatLottie: React.FC<{ url: string, style?: React.CSSProperties }> = ({ url, style }) => {
 ...
 */
export const VideoTrongSuot: React.FC<{ src: string, style?: React.CSSProperties, playbackRate?: number, isDark?: boolean }> = ({ src, style, playbackRate = 1, isDark = false }) => {
  const videoRef = useRef<HTMLVideoElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
 ...
 */
export const ConTroChuot: React.FC = () => {
 ...
 */
export const CuonLenDau: React.FC = () => {
 ...
 */
export const HieuUngHienDan: React.FC<{ children: React.ReactNode }> = ({ children }) => {
 ...
 */
export const ChuChay: React.FC<{ words: string[] }> = ({ words }) => {
