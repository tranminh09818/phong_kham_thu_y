...
test('Dump hoa don page HTML', async ({ page }) => {
    ...
    require('fs').writeFileSync(path.join(OUT, 'debug_hoa_don.html'), html, 'utf8');
    await page.screenshot({ path: path.join(OUT, 'debug_hoa_don.png') });
    console.log('Dumped HTML and screen of bill page');
});
