...
test.describe('Kiểm tra Chatbot thông minh - ý định, media và giọng nói', () => {
  ...
  test('Ảnh gửi lên API giữ data URL và MIME type để AI nhận diện đúng định dạng', async ({ page }) => {
    ...
    await loginAsCustomer(page);
    await page.route('**/api/chat*', async route => { capturedBody = route.request().postDataJSON(); await route.fulfill({ status: 200, contentType: 'application/json; charset=utf-8', body: JSON.stringify({ reply: 'Đã nhận ảnh để phân tích.' }) }); }); await openChat(page); const png1x1 = Buffer.from( 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+/p9sAAAAASUVORK5CYII=', 'base64' ); await page.locator('input[data-ai-id="input-chatbot-jmt6"]').setInputFiles({ name: 'vet-sample.png', mimeType: 'image/png', buffer: png1x1 }); await expect(page.locator('#chatWindow')).toBeVisible(); await page.locator('textarea').first().fill('Nhìn ảnh này giúp t nhận diện tình trạng da của bé'); await page.locator('button[data-ai-id="button-chatbot-5x21"]').click({ force: true }); await expect.poll(async () => capturedBody || await page.evaluate(() => (window as any).__REXI_LAST_CHAT_PAYLOAD__ || null)).not.toBeNull(); capturedBody = capturedBody || await page.evaluate(() => (window as any).__REXI_LAST_CHAT_PAYLOAD__); const lastMessage = capturedBody[capturedBody.length - 1]; expect(lastMessage.images[0]).toMatch(/^data:image\/(png|jpeg);base64,/); }); test('Nhận diện giọng nói gửi đúng transcript vào chatbot', async ({ page }) => { await page.context().grantPermissions(['microphone']); await page.addInitScript(() => { class FakeSpeechRecognition { static instance: any; continuous = false; interimResults = false; lang = ''; onresult: any; onerror: any; onend: any; constructor() { FakeSpeechRecognition.instance = this; (window as any).__speechRecognitionInstance = this; } start() {} stop() { if (this.onend) this.onend(); } } (window as any).SpeechRecognition = FakeSpeechRecognition; (window as any).webkitSpeechRecognition = FakeSpeechRecognition; (window as any).__emitFinalSpeech = (text: string) => { const instance = (window as any).__speechRecognitionInstance; instance.onresult({ resultIndex: 0, results: [ { isFinal: true, 0: { transcript: text } } ] }); }; }); await loginAsCustomer(page); await page.route('**/api/chat', async route => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json; charset=utf-8',
